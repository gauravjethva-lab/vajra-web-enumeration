import subprocess
import os

from core.utils import require_tool


def clean_hosts(input_file, clean_file):

    hosts = set()

    if not os.path.exists(input_file):
        return

    with open(input_file, "r") as file:
        for line in file:
            line = line.strip()
            if not line:
                continue
            line = line.replace("https://", "").replace("http://", "")
            line = line.split("/")[0].split(":")[0]
            if line:
                hosts.add(line)

    with open(clean_file, "w") as file:
        for host in sorted(hosts):
            file.write(host + "\n")


def scan_ports(domain):

    input_file = f"output/{domain}/live_subdomains.txt"
    clean_hosts_file = f"output/{domain}/clean_hosts.txt"
    masscan_output = f"output/{domain}/masscan.txt"
    naabu_output = f"output/{domain}/naabu.txt"
    final_output = f"output/{domain}/open_ports.txt"

    print("\n[+] Preparing Hosts For Port Scanning...")

    clean_hosts(input_file, clean_hosts_file)

    if not os.path.exists(clean_hosts_file):
        print("[-] No hosts to scan.")
        open(final_output, "w").close()
        return

    # =========================================================
    # MASSCAN
    # =========================================================

    masscan_bin = require_tool("masscan")

    if masscan_bin:
        print("[+] Running Masscan...")
        sudo_prefix = "" if os.geteuid() == 0 else "sudo "
        subprocess.run(
            f"{sudo_prefix}{masscan_bin} -p1-1000 --rate 1000 -iL {clean_hosts_file} -oL {masscan_output} > /dev/null 2>&1",
            shell=True, timeout=300
        )

    # =========================================================
    # NAABU
    # =========================================================

    naabu_bin = require_tool("naabu")

    if naabu_bin:
        print("[+] Running Naabu...")
        subprocess.run(
            f"cat {clean_hosts_file} | {naabu_bin} -top-ports 1000 -silent -o {naabu_output} > /dev/null 2>&1",
            shell=True, timeout=300
        )

    # =========================================================
    # MERGE RESULTS
    # =========================================================

    print("[+] Merging Port Results...")

    combined_ports = set()

    if os.path.exists(naabu_output):
        with open(naabu_output, "r") as f:
            for line in f:
                line = line.strip()
                if line:
                    combined_ports.add(line)

    if os.path.exists(masscan_output):
        with open(masscan_output, "r") as f:
            for line in f:
                if "open" in line:
                    parts = line.split()
                    try:
                        port = parts[2].split("/")[0]
                        host = parts[3]
                        combined_ports.add(f"{host}:{port}")
                    except IndexError:
                        pass

    with open(final_output, "w") as f:
        for item in sorted(combined_ports):
            f.write(item + "\n")

    print(f"[+] Open ports saved to {final_output}")
    print("\n[+] Port Scanning Completed!")
