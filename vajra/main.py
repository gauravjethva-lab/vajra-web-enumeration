...
def print_stage(num, total, title, icon):
    ...
    console.print(
        f"[bold yellow] {icon} Stage [{num}/{total}] : {title}[/bold yellow]"
    )
    console.print(
        f"[bold bright_cyan]{'─'*50}[/bold bright_cyan]\n"
    )
...
def print_final_summary(domain, timings):
    ...
    table.add_column("Stage", style="bold cyan", min_width=25)
    ...
    console.print(
        Panel(
            ...
            f"[bold yellow]📊 Generate HTML Report:[/bold yellow]\n"
            f"[white]  python3 report_generator.py {domain}[/white]\n\n"
            f"[bold yellow]📋 Generate Markdown Summary:[/bold yellow]\n"
            f"[white]  python3 recon_summary.py {domain}[/white]",
